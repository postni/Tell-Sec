...
ipcRenderer.on('scanner-reply',(event, d) => {
    console.log(d);
    console.log("============================================")
    console.log("--------------------------------------------")
    console.log("============================================")
    console.log(JSON.parse(d));
    fillTable(JSON.parse(d));
  })
  ...
  function fillTable(data){
    ...
    for(item in data){
        ...
        col2.appendChild(document.createTextNode(
            (data[item][0]["hostname"]===null) ? "unbekannt" : data[item][0]["hostname"]
        ));
        let col3 = document.createElement("td");
        col3.appendChild(document.createTextNode(
            (data[item][0]["vendor"]===undefined)? "unbekannt": data[item][0]["vendor"]
        ));
        ...
        col6.appendChild(document.createTextNode(
            (data[item][0]["mac"]===null)?"unbekannt":data[item][0]["mac"]
        ));
